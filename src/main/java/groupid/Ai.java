...
public class Ai extends SwingWorker<Void, String>{
    ...
    public Ai(GameInterface gameInterface){
        ...
        //!debug: {custom paneli yapmak icin kapattim simdilik}
...
//?==========================================================================================================================================
    public void brainDiceAlgo(){ //! tankSolver()
        ...
        boolean doWeHaveToGuess = true;
        long[][] probabilityBoard = new long[gameInterface.getSatirSize()][gameInterface.getSutunSize()]; 
        //? this board will store the all the empty tile counts
        ...
        for(int recursiveTiles_i = 0; recursiveTiles_i < seperateRecursiveTilesList.size(); recursiveTiles_i++){
            ...
            boolean[] enumarationArr = new boolean[seperateRecursiveTilesList.get(recursiveTiles_i).size()];
            backtrackingAlgoRecurse(seperateRecursiveTilesList.get(recursiveTiles_i), enumarationArr, 0, userBoardSnapShot);

            for(int i = 0; i < enumarationArr.length; i++){

                boolean isAllEnumrationsMine = true;
                boolean isAllEnumarationsEmpty = true;

                Pair pairOfThisIteration = seperateRecursiveTilesList.get(recursiveTiles_i).get(i);

                for(int j = 0; j < this.backtrackingAlgoSolutions.size(); j++){

                    if(backtrackingAlgoSolutions.get(j)[i] == true){

                        isAllEnumarationsEmpty = false;
                    }
                    if(backtrackingAlgoSolutions.get(j)[i] == false){

                        isAllEnumrationsMine = false;
                        probabilityBoard[pairOfThisIteration.satir][pairOfThisIteration.sutun] ++;
                        //? her tile'in ne kadar empty oldugunun count'unu tutuyorum prob. board'da
                    }
                }

                if(isAllEnumrationsMine){

                    gameInterface.flagTile(pairOfThisIteration.satir, pairOfThisIteration.sutun);
                    //? kesin mine flagle
                }

                if(isAllEnumarationsEmpty){

                    gameInterface.openTile(pairOfThisIteration.satir, pairOfThisIteration.sutun);
                    //? kesin empty open'la

                    doWeHaveToGuess = false;
                    //? en az 1 tane bile kesin tile acarsak artik guessing yok demektir
                }
            }
        }
        ...
        if(!doWeHaveToGuess){
            ...
            return; //? En az bir tane kesin empty tile acildi zaten guessing'e gerek yok
        }
        else{
            ...
            Pair maxProbIndex = new Pair(0, 0);
            long maxProbSoFar = probabilityBoard[0][0];

            for(int i = 0; i < gameInterface.getSatirSize(); i++){

                for(int j = 0; j < gameInterface.getSutunSize(); j++){

                    if(probabilityBoard[i][j] > maxProbSoFar){

                        maxProbSoFar = probabilityBoard[i][j];
                        maxProbIndex.satir = i;
                        maxProbIndex.sutun = j;
                    }
                }
            }

            gameInterface.openTile(maxProbIndex.satir, maxProbIndex.sutun);
            //? Mecbur en yuksek prob'a sahip olan tile'i openla
        }
    }
//?==========================================================================================================================================
    public void backtrackingAlgoRecurse(ArrayList<Pair> recursiveTiles, boolean[] booleanArr, int currIndex, char[][] userBoardSnapShot){    //! tankRecurse()
        //? BooleanArr[] = {true : mine var | false : mine yok}

        char[][] dummyMineBoard = new char[gameInterface.getSatirSize()][gameInterface.getSutunSize()];
        ///? {mine var : 'm' | mine yok : '-'}
        ...
        for(int i = 0; i < userBoardSnapShot.length; i ++){
            ...
            for(int j = 0; j < userBoardSnapShot[0].length; j++){
                ...
                if(userBoardSnapShot[i][j] == 'f'){
                    ...
                    dummyMineBoard[i][j] = 'm';
                }
                if(isTileNumberUserBoardSnapshot(userBoardSnapShot, i, j)){ // usertBoard[i][j] == {0-8}

                    dummyMineBoard[i][j] = '-';
                }
            }
        }// once klasik userArrdaki bildiklerimi aktardim

        for(int i = 0; i < booleanArr.length; i ++){

            int satir_i = recursiveTiles.get(i).satir;
            int sutun_i = recursiveTiles.get(i).sutun;

            if(booleanArr[i] == true){

                dummyMineBoard[satir_i][sutun_i] = 'm';
            }
            else{

                dummyMineBoard[satir_i][sutun_i] = '-';
            }
        }// sonra bu recursive call'daki varsayimlari aktardim.

        int mineCountOfThisRecursiveCall = 0;

        for(int i = 0; i < booleanArr.length; i ++){

            if(booleanArr[i] == true){

                mineCountOfThisRecursiveCall ++;
            }
        }

        mineCountOfThisRecursiveCall = gameInterface.getFlagCount() + mineCountOfThisRecursiveCall;

        if(mineCountOfThisRecursiveCall > gameInterface.getMineNumber()){
            
            //?  Toplam mine count oyundaki mine count'u gecti infeasible !
            return;
        }
        ...
        for(int i = 0; i < gameInterface.getSatirSize(); i ++){
            ...
            for(int j = 0; j < gameInterface.getSutunSize(); j ++){
                ...
                if(! isTileNumberUserBoardSnapshot(userBoardSnapShot, i, j)){
                    ...
                    //? Zaten hakkind bir bilgim yon neyin infeasible'ligini kontrol edicem
                    continue;
                }
                ...
                int numberOnTile_ij = Character.getNumericValue(userBoardSnapShot[i][j]);
                
                if(dummyMineBoardCountMine(dummyMineBoard, i, j) > numberOnTile_ij){

                    return;
                    //?   mine count around that tile is incostistent with the number on it INFEASIBLE!
                }   
                if(adjacentTileCount - dummyMineBoardCountEmpty(dummyMineBoard, i, j) < numberOnTile_ij){

                    return;
                    //?  empty count around that tile is incostistent with the number on it INFEASIBLE!
                }
            }
        }

        if(currIndex == booleanArr.length){ //? A possible combination has been found

            if(isEndGame && mineCountOfThisRecursiveCall < gameInterface.getMineNumber()){

                //?   endGame case'de exact olarak mineCount bulmus olmamiz gerekiyor
                return;
            }

            this.backtrackingAlgoSolutions.add(booleanArr);
        }

        booleanArr[currIndex] = false;
        backtrackingAlgoRecurse(recursiveTiles, booleanArr, currIndex + 1, userBoardSnapShot);

        booleanArr[currIndex] = true;
        backtrackingAlgoRecurse(recursiveTiles, booleanArr, currIndex + 1, userBoardSnapShot);
    }
//?==========================================================================================================================================
